...
(function pageTransitions(){
  if(reduceMotion) return;
  const DURATION = 180; // keep in sync with the body.leaving transition in style.css
  document.addEventListener("click", e=>{
    if(e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
    const a = e.target.closest("a");
    if(!a || a.target === "_blank" || a.hasAttribute("download")) return;
    const href = a.getAttribute("href") || "";
    if(!href.endsWith(".html") || href === location.pathname.split("/").pop()) return;
    e.preventDefault();
...
(function constellation(){
  ...
  function frame(){
    if(document.body.classList.contains("leaving")){
      // freeze the heavy drawing during the page-out fade so it isn't fighting
      // the CSS transition for the same frames — we're about to navigate anyway
      if(!reduceMotion) requestAnimationFrame(frame);
      return;
    }
    ctx.clearRect(0,0,W,H);
    const LINK = 130*devicePixelRatio, LINK2 = LINK*LINK, MLINK = 170*devicePixelRatio, MLINK2 = MLINK*MLINK;
    for(const p of pts){
      p.x+=p.vx; p.y+=p.vy;
    ...
    for(let i=0;i<pts.length;i++){
      for(let j=i+1;j<pts.length;j++){
        // squared-distance pre-check avoids a sqrt for the common case of far-apart points
      ...
      const mdx=pts[i].x-mouse.x, mdy=pts[i].y-mouse.y, dm2=mdx*mdx+mdy*mdy;
      if(dm2<MLINK2){
...
/* hero fades, rises, and scales slightly as you scroll past it
   (single handler — used to be two competing scroll listeners both
   writing hero.style.transform, doubling the layout work for nothing) */
...
(function smoothScroll(){
  ...
  function loop(){
    current += (target - current) * 0.16;                  // the "butter" — was 0.085, felt sluggish/laggy
